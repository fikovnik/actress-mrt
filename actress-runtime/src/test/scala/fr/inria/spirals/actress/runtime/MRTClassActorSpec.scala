...
import fr.inria.spirals.actress.metamodel.{MRTFeature, MRTClass}
...
//      features should contain(MRTFeature("attribute2", classOf[String], observable = true, mutable = true))
      features should contain(MRTFeature("attribute3", classOf[String], container = true, unique = true))
      features should contain(MRTFeature("attribute4", classOf[String], container = true, unique = true, mutable = true))
...
//      features should contain(MRTFeature("reference2", classOf[R], reference = true, observable = true, mutable = true))
      features should contain(MRTFeature("reference3", classOf[R], reference = true, container = true, unique = true))
    ...
    "find feature getters in bindings" in {
      ...
      MRTClassActor.findGet(MRTFeature("attribute2", classOf[String]), classOf[B]) should be('defined)
      MRTClassActor.findGet(MRTFeature("attribute3", classOf[String], container = true, ordered = false, unique = true), classOf[B]) should be('defined)
...
//        def attribute1_=(v: String)
//
//        def attribute2: Set[String]
//
//        def attribute3: collection.mutable.Set[String]
//      }
//
